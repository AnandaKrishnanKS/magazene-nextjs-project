        ...
        <>
            <div className='grid lg:grid-cols-11 mx-6'>
                <div className='lg:col-span-2 mt-10 hidden lg:block'>
                    <div className='border mb-5 sticky top-0'>

                        <p className='text-[24px] font-bold m-5 text-[#4C4084]'>Business</p>
                        {businessData.slice(0, 5).map((item) =>
                            <div key={item.news_id} className='mx-5 border-b mb-5 pb-3'>
                                <p className='text-[18px] leading-5 font-gelasio font-bold hover:text-[#4C4084]'>
                                    {item.title}
                                </p>
                ...
                <div className='lg:col-span-6 md:px-8'>
                    ...
                    <div>
                        {politicsData.slice(0, 5).map((item) =>
                            ...
                            className='pb-7 grid grid-cols-9 space-x-3 md:space-x-4'>
                                <div className='col-span-3'>
                                    <div className='relative mt-2'>
                                        <Image aria-label='news related image' width={240} height={140} className='bg-slate-100 w-full ' src={item.image_path} alt="news image" />
                                        <p className='text-[12px] absolute bottom-0 left-0  bg-[#4C4084] text-white px-1'>{item.category}</p>
                                    </div>
                                </div>
                                <div className='md:mx-6 border-t mt-2 col-span-6'>
                                    <p className='text-[16px] md:text-[24px]  font-gelasio font-bold leading-6  md:my-3 hover:text-[#3b3169] '>{item.title}</p>
                                    <div className='text-[12px] md:mb-2 italic flex'>
                                        <span className='hidden md:block'  >{item.author} -</span><span> <HandleDate item={item}/></span>
                                    </div>
                                    <div className='text-[13px] text-[#3f3f3f] my-2 hidden md:block' dangerouslySetInnerHTML={{ __html: item.newsdetails.split(' ').slice(0, 16).join(' ') + "..." }} />
                                </div>
                            </Link>
                        )}
                    </div>
                ...
                <div className='lg:col-span-3 mt-10'>
                    <div className='border px-5'>
                        ...
                        {businessData.slice(0, 3).map((item) =>
                            <Link aria-label='link to detail page' href={`/${encodeURIComponent(item?.category)}/${encodeURIComponent(item?.title.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase().replace(/-+/g, '-').replace(/-+$/, ''))}`} key={item.news_id} 
                             className='grid grid-cols-8 my-6 space-x-1'>
                                ...
                                <div className='w-full h-auto object-cover col-span-3'>
                                    <Image aria-label='news related image' className='bg-gray-300 w-full' width={120} height={70} src={item.image_path} alt="round news image" />
                                </div>
                            </Link>
                        )}
                        <div className='px-4'>
